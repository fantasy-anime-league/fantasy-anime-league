...
@pytest.mark.parametrize('id,name,season_id,alias,derivative,eligible,season_of_year,year', [
    (36432, 'Toaru Majutsu no Index III', 1, None, True, 1, "fall", 2018),
    (36000, 'Sora to Umi no Aida', 1, 'test alias', False, 1, "fall", 2018)
])
def test_query_anime(id, name, season_id, alias, derivative, eligible, season_of_year, year):
    with session_scope(True) as session:
        ...
        assert anime.derivative == derivative
