...
def test_calculate_team_total_score(team_weekly_points_factory, orm_team_factory, session):
    ...
    assert team_score.calculate_team_total_score(team, session) == sum(
...
def test_aced_anime_earns_extra(
    ...
    config_function = config_functor(
        sections=["scoring info"], kv={"ace-cutoff": 60000, "ace-value": 3000}
    )
...
def test_calculate_team_scores_assigns_highest_team_correctly_and_adds_bonus(
    ...
    config_function = config_functor(
        sections=["season info", "weekly info", "scoring info"],
        kv={
            "season": "spring",
            "year": 2018,
            "current-week": 0,
    ...
    for anime_weekly_stat in anime_weekly_stats[:5]:
        team_weekly_anime_factory(
            team=teams[0], anime=anime_weekly_stat.anime, week=anime_weekly_stat.week
        )
    for anime_weekly_stat in anime_weekly_stats[3:8]:
        team_weekly_anime_factory(
            team=teams[1], anime=anime_weekly_stat.anime, week=anime_weekly_stat.week
        )
    for anime_weekly_stat in anime_weekly_stats[-5:]:
        team_weekly_anime_factory(
            team=teams[2], anime=anime_weekly_stat.anime, week=anime_weekly_stat.week
        )
    ...
    top_team = (
        ...
        .filter(TeamWeeklyPoints.team_id == teams[2].id)
        .one()
    )
