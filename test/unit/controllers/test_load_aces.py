...
def test_load_aces(
    ...
    config_function = config_functor(
        ...
        kv={"season": "spring", "year": 2018, "current-week": 2},
    )
    ...
    for week in range(3):
        for _anime in anime[:5]:
            team_weekly_anime_factory(team=teams[0], anime=_anime, week=week)

        for _anime in anime[-5:]:
            team_weekly_anime_factory(team=teams[1], anime=_anime, week=week)

    team0_previous_aced_anime = (
    ...
    load_aces.load_aces(
        [f"{teams[0].name} {anime[0].name}", f"{teams[1].name} {anime[7].name}"]
    )

    should_not_be_aced_again = (
        session.query(TeamWeeklyAnime)
        .filter(
